...
final class GameViewModel: ObservableObject {
        ...
        contentService.fetchContentFeed(matchId: match.id)
            ...
            .sink { [weak self] completion in
                ...
                if case .failure(let error) = completion {
                    Self.logger.error("Feed load failed for match \(self.match.id, privacy: .public): \(String(describing: error), privacy: .public)")
                    self.scheduleFeedRetryIfNeeded()
                }
                ...
                if !sorted.isEmpty {
                    ...
                    self.feedRetryCount = 0
                } else {
                    Self.logger.warning("Feed returned 0 playable items for match \(self.match.id, privacy: .public)")
                    self.isLoading = false
                    self.feedStatusMessage = "No playable videos yet. Retrying..."
                    self.scheduleFeedRetryIfNeeded()
                }
            }
    ...
    private func scheduleFeedRetryIfNeeded() {
        guard feedRetryCount < maxFeedRetries else {
            isLoading = false
            feedStatusMessage = "No playable videos found for this match."
            return
        }
        feedRetryCount += 1
        Self.logger.info("Scheduling feed retry \(self.feedRetryCount)/\(self.maxFeedRetries) for match \(self.match.id, privacy: .public)")
