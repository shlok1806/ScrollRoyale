...
export function BrainCharacter({ rotLevel, size = 80, showArms = true }: BrainCharacterProps) {
  const getEyeState = () => {
    if (rotLevel < 30) return 'happy';
    if (rotLevel < 50) return 'neutral';
    if (rotLevel < 70) return 'tired';
    return 'dead';
  };

  const getBrainColor = () => {
    if (rotLevel < 30) return { main: '#FFB3D1', dark: '#FF8FB8', accent: '#FF6B9D' };
    if (rotLevel < 50) return { main: '#E0B3D1', dark: '#C88FB8', accent: '#B06B9D' };
    if (rotLevel < 70) return { main: '#C88FB8', dark: '#A0708F', accent: '#805070' };
    return { main: '#A569BD', dark: '#8B4AA0', accent: '#6B2B7D' };
  };

  const eyeState = getEyeState();
  const colors = getBrainColor();
  const showFlies = rotLevel >= 50;
  const showSecondFly = rotLevel >= 75;
