...
export default function DuelResult() {
  ...
  // Match results (can be made dynamic)
  const isVictory = Math.random() > 0.4;
  const yourHP = isVictory ? 742 : 0;
  const opponentHP = isVictory ? 0 : 823;
  const trophyChange = isVictory ? 30 : -18;
  const xpGained = isVictory ? 185 : 95;
  const rankUp = isVictory && Math.random() > 0.7;

  // Match stats
  const stats = {
    damageDealt: 258,
    damageTaken: isVictory ? 258 : 1000,
    highestCombo: 5,
    videosCompleted: 7,
    discoveryPayouts: 3,
    quizzesCorrect: 4,
