...
const TOTAL_VIDEOS = 9; // 90 seconds / 10 seconds per video
...
export default function DuelArena() {
    ...
    const videoTimer = setInterval(() => {
      setVideoTimeLeft((prev) => {
        if (prev <= 1) {
          ...
          if (currentVideoIndex < TOTAL_VIDEOS - 1) {
            setCurrentVideoIndex(currentVideoIndex + 1);
            // Lock banked damage and apply
            applyBankedDamage();
          }
  ...
  // Simulate opponent damage
  useEffect(() => {
    const interval = setInterval(() => {
      if (Math.random() > 0.6) {
        const damage = Math.floor(Math.random() * 40) + 20;
        setYourHP(prev => Math.max(0, prev - damage));
        addDamagePopup(damage, true);
      }
    }, 4000);
  ...
  const handleSwipe = (direction: 'up' | 'down') => {
    if (direction === 'up' && currentVideoIndex < TOTAL_VIDEOS - 1) {
      setCurrentVideoIndex(currentVideoIndex + 1);
  ...
  const timerProgress = (timeLeft / MATCH_DURATION) * 100;
    ...
    <div className="h-screen relative text-white overflow-hidden" ref={containerRef}>
      ...
      >
        <AnimatePresence mode="wait">
          ...
          >
            <VideoPlaceholder videoNumber={currentVideoIndex + 1} />
          </motion.div>
        </AnimatePresence>
      </div>
...
function VideoPlaceholder({ videoNumber }: { videoNumber: number }) {
  return (
