...
// MARK: - Demo Content Service

/// Fetches reels directly from the `public.reels` table using anon key only.
/// Used for the demo mode ("OPEN DEMO REEL MATCH") without needing a live match.
final class DemoContentService: ContentServiceProtocol {
    private static let logger = Logger(subsystem: "com.scrollroyale.app", category: "DemoContentService")

    private let client: SupabaseClient
    private let limit: Int

    init(client: SupabaseClient, limit: Int = 12) {
        self.client = client
        self.limit = limit
    }
    ...
    func fetchContentFeed(matchId: String) -> AnyPublisher<[ContentItem], Error> {
        client.select(
            table: "reels",
            queryItems: [
                URLQueryItem(name: "select", value: "id,storage_path,duration_ms"),
                URLQueryItem(name: "active", value: "eq.true"),
                URLQueryItem(name: "order", value: "created_at.desc"),
                URLQueryItem(name: "limit", value: "\(max(1, limit))")
            ],
            accessToken: nil,  // anon key only — no Bearer token for public reels
            decodeAs: [ReelRow].self,
            requestLabel: "demo_reels"
        )
        .map { [client] rows -> [ContentItem] in
            rows.enumerated().compactMap { idx, row in
                guard !row.storagePath.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
                    return nil
                }
                let url = Self.resolvePublicURL(storagePath: row.storagePath, client: client)
                guard let url else {
                    Self.logger.warning("Could not resolve public URL for reel path: \(row.storagePath, privacy: .public)")
                    return nil
                }
                return ContentItem(
                    id: row.id,
                    videoURL: url,
                    duration: TimeInterval(max(1000, row.durationMs ?? 10000)) / 1000.0,
                    order: idx
                )
            }
        }
    ...
    private static func resolvePublicURL(storagePath: String, client: SupabaseClient) -> URL? {
        let trimmed = storagePath.trimmingCharacters(in: .whitespacesAndNewlines)
        // If it's already an absolute URL, use it directly
        if trimmed.lowercased().hasPrefix("http://") || trimmed.lowercased().hasPrefix("https://") {
            return URL(string: trimmed)
        }
        // Split into bucket/path — e.g. "reels/v013.mp4" → bucket="reels", path="v013.mp4"
        let cleaned = trimmed.trimmingCharacters(in: CharacterSet(charactersIn: "/"))
        let parts = cleaned.split(separator: "/", maxSplits: 1).map(String.init)
        if parts.count == 2 {
            return client.createPublicStorageURL(bucket: parts[0], path: parts[1])
        }
        // Bare filename — assume "reels" bucket
        return client.createPublicStorageURL(bucket: "reels", path: cleaned)
    }
}

private struct ReelRow: Decodable {
    let id: String
